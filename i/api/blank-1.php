...
<head>
	...
	<title>空白文档① - <?php echo $website_array["website_banner"];?></title>
...
<div style="box-shadow: 5px 5px 25px 0 rgba(46,61,73,.2);border-radius:15px;font-size:13px;width:950px;font-family:微软雅黑,'Helvetica Neue',Arial,sans-serif;margin:10px auto 0px;border:0px solid #eee;max-width:100%;">
    <div style="width:100%;background-color: #3174ed;background-image: linear-gradient(90deg, #3174ed 0%, #FA8BFF 35%, #3fd9fb 88%);color:#FFFFFF;border-radius:15px 15px 0 0;">
        <h2 style="font-size:15px;word-break:break-all;padding:20px 32px;margin:0;text-align:center">空白文档① - <?php echo $website_array["website_banner"];?></h2>
    </div>
    <div style="margin:0px auto;width:90%">
		...
		<hr>
		# 请求地址 #<br/>
		<a style="color:#fd4b5c;text-decoration:none;"><?php echo $website_array["website_url"];?>/api/<?php echo basename(dirname(__FILE__))?>/api.php</a><br/>
		# 参数 #<br/>
		<li>url //必填)</li> 
		<li>dm //必填)</li> 
		<hr>
		...
		本API无返回数据,直接输出图片 {"code":"200","主办单位名称":"深圳市腾讯计算机系统有限公司","主办单位性质":"企业","网站备案/许可证号":"粤B2-20090059-5","网站名称":"腾讯网","网站首页网址":"www.qq.com","审核时间":"2018/11/9 0:00:00"}<br/>
		</div>
		<hr>
		# 备注 #<br/>
          <div style="width:100%;background-color: #000;background-image: linear-gradient(90deg, #6694ed 0%,);color:#FFFFFF;border-radius:5px">
       ...
       <&#63php <br/>
$dm = $_GET['dm'];<br/>
$str = file_get_contents('<?php echo $website_array["website_url"];?>/api/<?php echo basename(dirname(__FILE__))?>/api.php?url=www.liecnk.com'.$dm);<br/>
...
&#60!--HTML代码结束-->
		...
		<hr>
		# 示例 #<br/>
		<a style="color:#fd4b5c;text-decoration:none；" target="_blank"><?php echo $website_array["website_url"];?>/api/<?php echo basename(dirname(__FILE__))?>/api.php?url=www.liecnk.com</a>
		</h3>
		</div>
          </div>
		</div>
